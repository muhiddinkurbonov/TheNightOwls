...
# This script should be run from the project root: ./scripts/cicd.sh
API_PROJECT_DIR="./api"
...
run_tests_coverlet() {
    ...
    dotnet test ${TEST_PROJECT_DIR} \
        ...
        --settings ${TEST_PROJECT_DIR}/.runsettings
...
run_api() {
    ...
    # Kill any existing process on port 5288
    status_message "$YELLOW" "Checking for existing processes on port 5288..."

    if [[ "$OS_ENV" == "windows" ]]; then
        # Windows: Find and kill process using port 5288
        local pid=$(netstat -ano | grep ":5288" | grep "LISTENING" | awk '{print $5}' | head -1)
        if [ -n "$pid" ]; then
            status_message "$YELLOW" "Found existing process (PID: $pid) on port 5288. Terminating..."
            taskkill //F //PID "$pid" 2>/dev/null || true
            sleep 1
            status_message "$GREEN" "Existing process terminated."
        fi
    else
        # Linux/macOS: Find and kill process using port 5288
        local pid=$(lsof -ti:5288 2>/dev/null)
        if [ -n "$pid" ]; then
            status_message "$YELLOW" "Found existing process (PID: $pid) on port 5288. Terminating..."
