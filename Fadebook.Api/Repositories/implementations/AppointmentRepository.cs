    ...
    ) : IAppointmentRepository
{
    ...
    public async Task<IEnumerable<AppointmentModel>> GetByBarberIdAndDateAsync(int barberId, DateTime targetDate)
    {
        // Ensure we're comparing dates in UTC
        var startOfDay = DateTime.SpecifyKind(targetDate.Date, DateTimeKind.Utc);
        var endOfDay = startOfDay.AddDays(1);
        ...
        return await _fadebookDbContext.appointmentTable
            ...
            .Include(a => a.Barber)
            .Include(a => a.Service)
            .Where(a => a.BarberId == barberId && a.AppointmentDate >= startOfDay && a.AppointmentDate < endOfDay)
            .ToListAsync();
    }
